...
task svpack_filter_annotated {
	...
	runtime {
		docker: "quay.io/pacbio/svpack:36180ae6f271b93964250bd05f7e61842ffb6cb3_build3"
		cpu: 4
    memory: "16 GB"
    disk: file_size + " GB"
    maxRetries: 2
...
task cpg_pileup {
  ...
  runtime {
    docker: "quay.io/pacbio/pb-cpg-tools:v2.3.2_build2"
    cpu: threads
    memory: "~{threads * 4} GB"
    disk: file_size + " GB"
    maxRetries: 2
...
task mutationalpattern {
  ...
  runtime {
      docker: "quay.io/pacbio/somatic_r_tools:v0.9.2"
      cpu: threads
      memory: "~{threads * 4} GB"
      disk: file_size + " GB"
      maxRetries: 2
